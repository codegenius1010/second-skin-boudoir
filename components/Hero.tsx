...
export function Hero({ eyebrow, title, subtitle, primary = 'Book Your Private Consultation', secondary, image }: { eyebrow?: string; title: string; subtitle: string; primary?: string; secondary?: { label: string; href: string }; image?: string }) {
  return <section className="relative flex min-h-[86vh] items-center overflow-hidden px-5 pt-28 lg:px-8">
    ...
    <div className="relative z-10 mx-auto max-w-7xl py-20">
      ...
      <div className="mt-9 flex flex-col gap-4 sm:flex-row"><Button href="/contact">{primary}</Button>{secondary && <Button href={secondary.href} variant="secondary">{secondary.label}</Button>}</div>
